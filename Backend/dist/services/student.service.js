...
export const getAllStudentsService = async (page = 1, limit = 10, mentorId) => {
...
export const updateStudentService = async (id, data, mentorId) => {
...
export const deleteStudentService = async (id, mentorId) => {
    if (mentorId) {
        const student = await prisma.student.findUnique({ where: { id } });
        if (!student || student.mentorId !== mentorId) {
            throw new Error("Student not found or unauthorized");
...
export const getStudentProgressService = async (studentId, mentorId) => {
